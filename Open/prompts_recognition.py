# Notes:
# 1. Add figure-only prompts
# 2. Add figure+text prompts
# 3. Add finetune related prompts to each (later)
# note that sometimes there is a need to add information beyond the figure
# such information are in data, not here in prompts
...
# Overall Figure only and figure+text
FIGURE_ONLY = "Data of the problem is provided in the figure and additional information, if have, will be provided at the end."
FIGURE_TEXT = "Data of the problem is provided in the figure and the text version is provided at the end."

# Recognition
Initial_check = "Here is the textual description of a problem and a figure of a problme. Please check if they describe the same question."
Check_comply = "If so, please output 'YES,' and provide by the step-by-step checking process; if not, please output 'NO,' followed by the reason."
...
kspPrompts = {
    "Intro": "The Knapsack Problem (KSP) asks whether a subset of items, each with a given weight, can be chosen to fit into a knapsack of fixed capacity, maximizing the total value without exceeding the capacity.",
    "Initial_question": "Determine if a subset of items can be selected to fit into a knapsack with a capacity of {knapsack_capacity}, maximizing weight without exceeding the capacity. Item weights are presented by the top number after 'W:'. Item ID are shown in as numbers below the weights after 'id:'. ",
    "Figure_only": FIGURE_ONLY,
    "Figure_text": FIGURE_TEXT,
